...
void aprintf(char *format, ...) {
	...
	vsnprintf(putbuf, AVIO_BUFFER+1, format, args);
	va_end(args);
...
MRESULT EXPENTRY FontDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	...
	case WM_INITDLG:
		...
		for (i = 0; i < Fonts; i++)
		{
			...
			{
				snprintf(buf, 256, "%s %ux%u", fonts[i].szFacename, (unsigned int)fonts[i].lMaxBaselineExt, (unsigned int)fonts[i].lAveCharWidth);
	...
	case DID_OK:
		...
		if(i2)
		{
			snprintf(fontsize, 10, "%dx%d", (int)fonts[i].lAveCharWidth, (int)fonts[i].lMaxBaselineExt);
...
int mesg(char *format, ...) {
	...
	va_start(args, format);
	vsnprintf(outbuf, 256, format, args);
	va_end(args);
		...
		case SETS1:
			...
			if(p1e!=LIT_NONE)
			{
				...
				case INT_TYPE_VAR:
					...
					snprintf(szBuffer, 1024, "%d", get_int_var(p1e));
		 ...
		 case SETS2:
			 ...
			 if(p2e!=LIT_NONE && chan)
			 {
				 ...
				 case INT_TYPE_VAR:
					 ...
					 snprintf(szBuffer, 1024, "%d", get_cset_int_var(chan->csets, p2e));
...
MRESULT	EXPENTRY NotebookDlgProc(HWND hWnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	...
	case WM_INITDLG:
		...
		if(nb_window->current_channel)
			snprintf(szBuffer, 200, "PMBitchX Properties for %s", nb_window->current_channel);
		else
...
void gui_query_window_info(Screen *screen, char *fontinfo, int *x, int *y, int *cx, int *cy)
{
	...
	snprintf(fontinfo, 20, "%dx%d", screen ? screen->VIO_font_width : 0, screen ? screen->VIO_font_height : 0);
}
...
void gui_setfileinfo(char *filename, char *nick, int server)
{
	...
	snprintf(buffer, 200, "%s@%s %s", nick, s ? s : "IRC", t ? t : "clock problem");
